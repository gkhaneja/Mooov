...
class UserDetailsService extends RestService
{
 ...
 public function getInfo($arguments){
  ...
  if(isset($arguments['insta']) && $arguments['insta'] == 0){
   $sql = "select * from carpool where user_id = " . $arguments['user_id'];
   $result = $dbobject->execute($sql);
   if($result->num_rows > 0){
   ...
   $requestobj->showMatches(array(array('match' => $arguments['user_id'], 'details' => $details, 'request' => $request)), 0, 1);
  }else{
   $sql = "select * from request where user_id = " . $arguments['user_id'];
   $result = $dbobject->execute($sql);
   if($result->num_rows > 0){
    $request = $result->fetch_assoc();
   }else{
   ...
   $requestobj->showMatches(array(array('match' => $arguments['user_id'], 'percent'=>100,'details' => $details, 'request' => $request)), 0, 0);
  }
 }
